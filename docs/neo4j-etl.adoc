...
It supports *MySQL*, *PostgreSQL* and *Oracle* relational databases.

You can get the http://github.com/neo4j-contrib/neo4j-etl-components/releases/latest[latest version of the import tool from GitHub].

Once downloaded and uncompressed the zip, you also need download the proper JDBC Driver and add it to the `lib` folder.
...
----
export NEO4J_HOME=/path/to/neo4j-enterprise-3.1.3

mkdir -p /tmp/northwind

./bin/neo4j-etl { mysql | postgresql | oracle } generate-metadata-mapping --database northwind --user root > /tmp/northwind/mapping.json
# Creating RDBMS to CSV mappings...

echo '{"multiline-fields":"true"}' > /tmp/northwind/options.json

./bin/neo4j-etl { mysql | postgresql | oracle } export --database northwind --user root \
...
# /Users/mh/v/neo4j-enterprise-3.1.3/data/databases/graph.db
...
////
./neo4j-etl { mysql | postgresql | oracle } export --host 127.0.0.1 --user neo4j --password neo4j \
--database javabase --destination /tmp/neo4j-enterprise-3.0.1/data/databases/graph.db/ \
--import-tool /tmp/neo4j-enterprise-3.1.3/bin  --csv-directory ./  \
...
----
include::neo4j-etl-rdbms-generate-metadata-mapping.txt[]
----

=== 'export' Command

----
include::neo4j-etl-rdbms-export.txt[]
----
...
- https://raw.githubusercontent.com/neo4j-contrib/neo4j-etl-components/master/neo4j-etl-it/src/main/resources/scripts/mysql/northwind.sql[MySQL]
- https://raw.githubusercontent.com/neo4j-contrib/neo4j-etl-components/master/neo4j-etl-it/src/main/resources/scripts/postgresql/northwind.sql[PostgreSQL]
- https://raw.githubusercontent.com/neo4j-contrib/neo4j-etl-components/master/neo4j-etl-it/src/main/resources/scripts/oracle/northwind.sql[Oracle]
...
https://raw.githubusercontent.com/neo4j-contrib/neo4j-etl-components/master/neo4j-etl-it/src/main/resources/scripts/mysql/northwind.sql
...
~: ./bin/neo4j-etl mysql export --url jdbc:mysql://localhost:5433/northwind?autoReconnect=true&useSSL=false \
      --user neo4j --password neo4j --import-tool $NEO4J_HOME/bin \
      --options-file /tmp/northwind/options.json --csv-directory /tmp/northwind \
      --destination $NEO4J_HOME/data/databases/graph.db/ --quote '"' --force
----
...
Load the database via the following sql script:
https://raw.githubusercontent.com/neo4j-contrib/neo4j-etl-components/master/neo4j-etl-it/src/main/resources/scripts/postgresql/northwind.sql
...
~: ./bin/neo4j-etl postgresql export --url jdbc:postgresql://localhost:5433/northwind?ssl=false \
      --user neo4j --password neo4j --import-tool $NEO4J_HOME/bin \
      --options-file /tmp/northwind/options.json --csv-directory /tmp/northwind \
      --destination $NEO4J_HOME/data/databases/graph.db/ --quote '"' --force
----
...
root@692c446a274b:~# wget https://raw.githubusercontent.com/neo4j-contrib/neo4j-etl-components/master/neo4j-etl-it/src/main/resources/scripts/oracle/northwind.sql
...
~: ./bin/neo4j-etl oracle export --url jdbc:oracle:thin:@localhost:49161:XE \
      --schema northwind --user northwind --password northwind --import-tool $NEO4J_HOME/bin \
      --options-file /tmp/northwind/options.json --csv-directory /tmp/northwind \
      --destination $NEO4J_HOME/data/databases/graph.db/ --quote '"' --force
----
