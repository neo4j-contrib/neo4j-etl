...
----
include::partial$neo4j-etl-generate-metadata-mapping.txt[]
----

=== 'export' command

----
include::partial$neo4j-etl-export.txt[]
----
...
include::partial$etl_config.txt[]
----
...
in order to support this feature we added a select options box in the import view as it follows:
...
- 'cypher:batch' (which creates CSV files in the same way as cypher:direct but it
translate them into UNWIND statements instead of LOAD CSV)
...
- enable property `dbms.security.allow_csv_import_from_file_urls` and set it to true in order to allow
LOAD CSV to read CSV files from external resource
- remove property `dbms.directories.import=import` or comment it out to prevent Neo4j to search for CSV file
into the default import folder
