...
----
include::neo4j-etl-rdbms-ui.txt[]
----
...
=== Edit Metadata Mapping via User Interface

----
STEP 1: Generate Metadata Mapping File (via command line)
...
You can use any of the below alternatives, but don't forget to add the following command option:
  ...
  --mapping-file /tmp/northwind/edited-mapping.json
